        ...
        <div className='mx-auto w-11/12 max-w-[1000px] py-10'>
            ...
            <div className='flex items-center justify-between rounded-md border-[1px] border-richblack-700 bg-richblack-800 md:p-8 md:px-12 px-3 py-3 text-richblack-5'>
                <div className=' flex items-center gap-x-4'>
                    <img src={profilePicture} className='aspect-square w-[78px] rounded-full object-cover'/>
                    <div className='space-y-2'>
                        <p>Change Profile Picture</p>
                        <form onSubmit={handleUpload}>
                            <div className='flex flex-row gap-3'>
                                <label htmlFor="upload" className=' cursor-pointer rounded-md bg-richblack-700 py-2 px-5 font-semibold text-richblack-50'>
                                    Select <input type="file" id='upload' onChange={handleFilechange} className='hidden' accept='image/png,image/gif, image/jpeg'/>
                                </label>
                                <button type='submit' className=' flex items-center bg-yellow-50 cursor-pointer gap-x-2 rounded-md py-2 px-5 font-semibold text-richblack-900 undefined'>Upload <HiOutlineUpload/></button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div className='my-10 flex flex-col gap-y-6 rounded-md border-[1px] border-richblack-700 bg-richblack-800 p-8 px-12'>
                    ...
                    <div className='flex flex-col gap-5 lg:flex-row'>
                        <div className='flex flex-col gap-2 lg:w-[48%]'>
                            ...
                            <input type="text" defaultValue={user.firstName ||null} name='firstName' id='firstName' placeholder='Enter First Name' className='form-style' onChange={handleChange}/>
                        </div>
                        ...
                        <div className='flex flex-col gap-2 lg:w-[48%]'>
                            ...
                            <input type="text" defaultValue={user.lastName ||null} name='lastName' id='lastName' placeholder='Enter Last Name' className='form-style' onChange={handleChange}/>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-5 lg:flex-row">
                        <div className="flex flex-col gap-2 lg:w-[48%]">
                            ...
                            <input defaultValue={user?.additionalDetails.dateOfBirth || null} type="date" name="dateOfBirth" id="dateOfBirth" className="form-style" onChange={handleChange}/>
                        </div>
                        <div className="flex flex-col gap-2 lg:w-[48%]">
                            <label htmlFor="gender" className="text-richblack-50">Gender</label>
                            <select defaultValue={user?.additionalDetails.gender || null} type="text" name="gender" id="gender" className="form-style" onChange={handleChange}>
                    ...
                    <div className="flex flex-col gap-5 lg:flex-row">
                        <div className="flex flex-col gap-2 lg:w-[48%]">
                            ...
                            <input defaultValue={user?.additionalDetails.contactNumber || null} type="tel" name="contactNumber" id="contactNumber" placeholder="Enter Contact Number" className="form-style" onChange={handleChange}/>
                        </div>
                        <div className="flex flex-col gap-2 lg:w-[48%]">
                              <label htmlFor="about" className="text-richblack-50">About</label>
                              <input defaultValue={user?.additionalDetails.about || null} type="text" name="about" id="about" placeholder="Enter Bio Details" className="form-style" onChange={handleChange}/>
                        </div>
                    </div>
                </div>
                <div className='flex justify-end gap-2'>
                    <button className='flex items-center bg-yellow-50 cursor-pointer gap-x-2 rounded-md py-2 px-5 font-semibold text-richblack-900 undefined' type='submit'>
                        Save
                    </button>
                </div>
            </form>
                    ...
                    <div className='relative mt-4'>
                        <label className='w-full'>
                            <p className='mb-1 text-[0.875rem] leading-[1.375rem] text-richblack-5'>
                                Old Passwrod <sup className='text-pink-200 '>*</sup>
                            </p>
            ...
            <div className="my-10 flex flex-row gap-x-5 rounded-md border-[1px] border-pink-700 bg-pink-900 p-3 md:p-8 md:px-12">
                ...
                <div className="flex flex-col space-y-2 w-full">
                    ...
                    <button type="button" onClick={onDeleteAccount} className="w-fit cursor-pointer italic text-pink-300">I want to delete my account.</button>
                </div>
            </div>
        </div>
      </div>
