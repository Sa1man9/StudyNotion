                            ...
                            <div className=' flex flex-col items-end pr-4'>
                                {
                                    subLinks?.length < 0 ? (<div></div>) : (
                                        subLinks?.map((element, index) => (
                                            <Link to={`/catalog/${element?.name.split(" ").join("-").toLowerCase()}`} key={index} onClick={() => { dispatch(setProgress(30)); shownav() }} className="p-2 text-sm">
...
{/* desktop */}
        ...
        <div className='flex-row gap-5 hidden md:flex items-center '>
            ...
            {
                token === null &&(
                    <Link to="/login" className='text-richblack-25 ' onClick={()=>{setProgress(100)}}>
            ...
            {
                token !==null && (
                    <div className='pt-2'> <ProfileDropDown/></div>
                )
            }
        </div>
      </div>
